"""
video_creator.py v13
Complete fix based on full audit:
- Content fills full screen height (not just top 40%)
- Character name badge moved above head, not over feet
- Scene flash transition between scenes
- PT shown only once, cleanly
- Progress dots repositioned above character
- Sun scene header fixed
- Intro tagline centered
- 8 scenes x 42 frames = 14s
"""
...
TOTAL_FRAMES = sum(SCENE_FRAMES)  # 336 = 14s
...
def draw_om_watermark(img, alpha=8):
...
def add_scene_flash(img, f_in_scene):
    """White flash for first 3 frames of each scene — signals scene change."""
    if f_in_scene >= 3:
        return img
    alpha = int(180 * (1 - f_in_scene / 3))
...
def paste_char(base_img, frame_idx, scene):
    ...
    # Name badge ABOVE character's head — not over feet
    d = ImageDraw.Draw(base_img)
    bw, bh = 210, 36
    bx = CHAR_X - bw // 2
    badge_y = py - bh - 8   # above head
...
# ── SCENE RENDERERS ───────────────────────────────────────────────────────────
# RULE: ALL composites FIRST → draw = ImageDraw.Draw(img) → draw text
...
def scene_intro(img, f, panchang):
    ...
    # Om
    draw.text((CX, 85), "ఓం", font=get_font(78, bold=True),
              fill=GOLD + (fa,), anchor="mm")

    # Namaskaram pill
    draw_card(draw, PAD, 140, PAD+CARD_W, 230,
              fill=(75, 28, 0), border=SAFFRON, alpha=min(fa, 225))
    draw_mixed(draw, (CX, 185), "నమస్కారం!", 54, bold=True,
               fill=GOLD + (fa,), anchor="mm")

    # Host intro pill
    draw_card(draw, PAD, 248, PAD+CARD_W, 328,
              fill=(55, 20, 0), border=(160, 80, 0), alpha=min(fa, 210))
    draw_mixed(draw, (CX, 288), "నేను మీ పంచాంగం గురువు", 36, bold=True,
               fill=CREAM + (fa,), anchor="mm")

    # City — largest text
    draw_mixed(draw, (CX, 430), city, 72, bold=True,
    ...
    # Tagline — centered
    draw_mixed(draw, (CX, 660), "నేటి పంచాంగం వివరాలు చూద్దాం!",
...
def scene_tithi(img, f, panchang):
    ...
    # Label
    draw_card(draw, PAD, 105, PAD+CARD_W, 198,
              fill=(65, 25, 0), border=SAFFRON, alpha=min(fa, 228))
    draw_mixed(draw, (CX, 151), "తిథి", 58, bold=True,
               fill=SAFFRON + (fa,), anchor="mm")
    ...
    draw_card(draw, PAD, 215, PAD+CARD_W, 435,
              fill=(42, 14, 0), border=GOLD, radius=20, alpha=min(fa, 240))
    draw_mixed(draw, (CX, 308), p1, 56, bold=True,
               fill=GOLD + (fa,), anchor="mm")
    if p2:
        draw_mixed(draw, (CX, 390), p2, 38, fill=CREAM + (fa,), anchor="mm")

    # Paksha badge
    draw_card(draw, PAD+60, 452, PAD+CARD_W-60, 526,
              fill=(55, 18, 0), border=(175, 135, 0), radius=16, alpha=min(fa, 215))
    draw_mixed(draw, (CX, 489), paksha, 34, fill=CREAM + (fa,), anchor="mm")

    # TZ — clean single display
...
def _info_scene(img, f, label, time_val, tz, subtext, accent, bg_dark, pulse=False):
    """Full-screen info scene. Content spread across full height."""
    fa = fade(f, 10)
    ...
    # Label card — top section
    draw_card(draw, PAD, 105, PAD+CARD_W, 200,
              fill=bg_dark, border=accent, alpha=min(fa, 230))
    draw_mixed(draw, (CX, 152), label, 54, bold=True,
               fill=accent + (fa,), anchor="mm")

    # Time card — large center, pushed DOWN for visual balance
    ...
    draw_mixed(draw, (CX, 325), clean, tsize, bold=True,
               fill=accent + (fa,), anchor="mm")

    # TZ — once only, clean
    draw_mixed(draw, (CX, 447), tz, 30, fill=DIM + (fa,), anchor="mm")

    # Subtext card — lower section, large enough to read
    if subtext:
        ...
        draw_mixed(draw, (CX, 520), subtext, 34, bold=True,
                   fill=WHITE + (fa,), anchor="mm")
    return img


def scene_rahu(img, f, panchang):
    return _info_scene(img, f, "రాహు కాలం",
        tf(panchang, "rahukaal"), panchang.get("tz_label", "ET"),
        "ఈ సమయంలో కొత్త పని వద్దు!",
        WARN_RED, DARK_RED, pulse=True)


def scene_durmuhurtam(img, f, panchang):
    return _info_scene(img, f, "దుర్ముహూర్తం",
        tf(panchang, "durmuhurtam"), panchang.get("tz_label", "ET"),
        "శుభ కార్యాలు ఈ వేళ వద్దు!",
        WARN_RED, DARK_RED, pulse=True)


def scene_brahma(img, f, panchang):
    return _info_scene(img, f, "బ్రహ్మ ముహూర్తం",
        tf(panchang, "brahma_muhurta"), panchang.get("tz_label", "ET"),
        "ప్రార్థన & ధ్యానానికి శ్రేష్ఠ సమయం",
        GOLD, (55, 35, 0), pulse=False)


def scene_abhijit(img, f, panchang):
    return _info_scene(img, f, "అభిజిత్ ముహూర్తం",
        tf(panchang, "abhijit"), panchang.get("tz_label", "ET"),
...
def scene_sun(img, f, panchang):
    ...
    # Header
    draw_card(draw, PAD, 105, PAD+CARD_W, 198,
              fill=(58, 22, 0), border=SAFFRON, alpha=min(fa, 228))
    draw_mixed(draw, (CX, 151), "సూర్యోదయం & సూర్యాస్తమయం",
               42, bold=True, fill=SAFFRON + (fa,), anchor="mm")

    # Sunrise block
    draw_card(draw, PAD, 216, PAD+CARD_W, 380,
              fill=(50, 18, 0), border=GOLD, radius=20, alpha=min(fa, 240))
    draw_mixed(draw, (CX, 258), "సూర్యోదయం", 38, bold=True,
               fill=CREAM + (fa,), anchor="mm")
    draw_mixed(draw, (CX, 335), sunrise, 66, bold=True,
               fill=GOLD + (fa,), anchor="mm")

    # Sunset block
    draw_card(draw, PAD, 398, PAD+CARD_W, 562,
              fill=(48, 15, 0), border=SAFFRON, radius=20, alpha=min(fa, 240))
    draw_mixed(draw, (CX, 440), "సూర్యాస్తమయం", 38, bold=True,
               fill=CREAM + (fa,), anchor="mm")
    draw_mixed(draw, (CX, 516), sunset, 66, bold=True,
               fill=SAFFRON + (fa,), anchor="mm")
...
def scene_closing(img, f, panchang):
    ...
    # Blessing — large, prominent
    draw_card(draw, PAD, 105, PAD+CARD_W, 288,
              fill=(65, 28, 0), border=GOLD, radius=20, alpha=min(fa, 232))
    draw_mixed(draw, (CX, 172), "మీకు శుభమైన రోజు కలగాలని",
               40, bold=True, fill=GOLD + (fa,), anchor="mm")
    draw_mixed(draw, (CX, 245), "ఆశిస్తున్నాను!", 50, bold=True,
               fill=SAFFRON + (fa,), anchor="mm")

    # CTA
    draw_card(draw, PAD, 308, PAD+CARD_W, 440,
              fill=(50, 20, 0), border=SAFFRON, radius=20, alpha=min(fa, 225))
    draw_mixed(draw, (CX, 355), "Daily Panchangam kosam",
               34, bold=True, fill=CREAM + (fa,), anchor="mm")
    draw_mixed(draw, (CX, 412), "Follow చేయండి!", 48, bold=True,
               fill=SAFFRON + (fa,), anchor="mm")

    # Like/share/subscribe
    draw_card(draw, PAD+20, 460, PAD+CARD_W-20, 536,
              fill=(38, 14, 0), border=(150, 115, 0), radius=14, alpha=min(fa, 205))
    draw_mixed(draw, (CX, 498), "Like | Share | Subscribe చేయండి",
               32, fill=CREAM + (fa,), anchor="mm")

    # Handle + blessing
    draw_mixed(draw, (CX, 558), "@PanthuluPanchangam",
               28, fill=GOLD + (fa,), anchor="mm")
    draw_mixed(draw, (CX, 612), "జయ శ్రీమన్నారాయణ!", 42, bold=True,
               fill=GOLD + (fa,), anchor="mm")
    return img
...
def build_frame(frame_idx, panchang):
    ...
    # Progress dots — positioned at H-52 above character badge area
    draw = ImageDraw.Draw(img)
    n = len(SCENE_FRAMES); ds = 26
    sx = CX - (n * ds) // 2
    cy_dot = H - 52
    for i in range(n):
        x = sx + i * ds
        if i == scene:
            ...
            draw.ellipse([x-6, cy_dot-6, x+6, cy_dot+6],
                         outline=(155, 120, 0), width=2)
    return img
...
def create_thumbnail(panchang, output_path):
    ...
    # Rahu card
    draw_card(draw, PAD, 512, PAD+CARD_W, 626,
              fill=(110, 0, 0), border=WARN_RED, radius=20, alpha=242)
    ...
    # Follow CTA
    draw_card(draw, PAD, H-172, PAD+CARD_W, H-72,
              fill=(52, 20, 0), border=SAFFRON, radius=18, alpha=222)
